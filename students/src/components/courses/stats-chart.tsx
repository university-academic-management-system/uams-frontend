...
import { Bar, BarChart, CartesianGrid, LabelList, Legend, Pie, PieChart, Rectangle, Sector, Tooltip, XAxis, YAxis } from "recharts"



const StatsChart = ({ level, semester }: { level: Level, semester: Semester | "ALL" }) => {
    const { data: resultResponse, isLoading } = useResults({ level, ...(semester !== "ALL" ? { semester } : {}) });
...
const GradeChart = ({ level, semester }: { level: string, semester: string }) => {
    const { data: resultResponse } = useResults({ level, ...(semester !== "ALL" ? { semester } : {}) });
    const response = resultResponse;
