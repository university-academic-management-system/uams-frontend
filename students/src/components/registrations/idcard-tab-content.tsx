    ...
    return (
        <Stack gap="4">
            {(data?.transcripts?.length || 0) > 0 && <Flex justify={"space-between"} h="12">
                <Heading>ID Card&nbsp;Applications</Heading>
                <CreateIdCardDialog />
            </Flex>}
            ...
            {data?.transcripts.length === 0 ? (
                <EmptyStateView
                    ...
                    description="You haven't requested any ID cards yet."
                    action={<CreateIdCardDialog />}
                />
            ) : (
...
const CreateIdCardDialog = () => {
    const { register, handleSubmit, control, reset, formState: { errors } } = useCreateTranscriptForm()
    const path = useLocation().pathname;

    const { data: paymentDetails, isLoading } = usePaymentDetails(PaymentType.ID_CARD_FEE);
    ...
    const onSubmit = (data: CreateTranscriptFormData) => {
        initializePayment({
            type: PaymentType.ID_CARD_FEE,
            redirectUrl: `${window.location.origin}/students${path}?tab=id-cards`,
                            ...
                            <Dialog.Body spaceY="4">
                                ...
                                <Stack gap="4" colorPalette={"accent"}>
                                    <Field.Root invalid={!!errors.purpose}>
                                        <Field.Label>Reason for ID Card</Field.Label>
                                        <Input
...
export default IDCardTabContent;
