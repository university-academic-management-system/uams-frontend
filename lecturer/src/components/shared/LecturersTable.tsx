...
const LecturersTable = ({ lecturers, isLoading }: LecturersTableProps) => {
  ...
  // Always render the table structure; inside body we handle loading / empty / data
  return (
    <Box>
      <Table.ScrollArea >
...
// ========== STUDENT DRAWER (unchanged – keep as is) ==========
          ...
          <Drawer.Content roundedLeft="xl" maxW="450px">
            ...
            <Box px="4" py="3" borderBottomWidth="1px" borderColor="gray.100">
              <Flex align="center" gap="2">
                ...
                {searchTerm && (
                  <Button size="sm" variant="ghost" onClick={() => setSearchTerm('')} color="gray.400" _hover={{ color: "gray.600" }}>
                    Clear
                  </Button>
                )}
              </Flex>
            ...
            <Drawer.Body spaceY="4" py="6" pb="24" overflowY="auto">
              {isLoading ? (
                ...
                <Flex direction="column" align="center" justify="center" py="20" gap="4">
                  <Box bg="gray.50" p="4" rounded="full"><UserRoundPen /></Box>
                ...
                <>
                  ...
                  {filteredStudents.length === 0 ? (
                    <Flex direction="column" align="center" justify="center" py="10" gap="2">
                      ...
                      <Button size="xs" variant="ghost" color="blue.600" onClick={() => setSearchTerm('')}>Clear search</Button>
                    </Flex>
                  ) : (
                        ...
                        >
                          <Flex align="flex-start" justify="space-between">
                            <Box flex="1">
                              ...
                              <Text fontSize="xs" color="gray.500" fontWeight="500">{student.email}</Text>
                              <Flex align="center" gap="2" mt="2">
                                <Box px="2" py="0.5" bg="gray.100" rounded="full">
                                  <Text fontSize="10px" fontWeight="700" color="gray.600">{student.matricNumber}</Text>
                                </Box>
                                {student.level && (
            ...
            {students && students.length > 0 && (
              <Box position="absolute" bottom="0" left="0" right="0" bg="white" borderTopWidth="1px" borderColor="gray.100" p="4" zIndex="10" borderBottomLeftRadius="xl">
                <Button colorScheme="blue" w="full" h="12" rounded="lg" fontWeight="700" onClick={handleAssign} disabled={selectedStudents.size === 0 || isAssigning || isSessionLoading || !activeSession?.id} loading={isAssigning || isSessionLoading} bg="blue.600" _hover={{ bg: "blue.700" }}>
            ...
            <Drawer.Content roundedRight="xl" maxW="450px">
              ...
              <Box px="4" py="3" borderBottomWidth="1px" borderColor="gray.100">
                <Flex align="center" gap="2">
                  ...
                  {searchTerm && (
                    <Button size="sm" variant="ghost" onClick={() => setSearchTerm('')} color="gray.400">
                      Clear
                    </Button>
                  )}
                </Flex>
              ...
              <Drawer.Body spaceY="4" py="6" overflowY="auto">
                {isAssignedLoading ? (
                  ...
                  <Flex direction="column" align="center" justify="center" py="20" gap="4">
                    <Box bg="gray.50" p="4" rounded="full"><User /></Box>
                    <Box textAlign="center">
                      <Text fontSize="sm" fontWeight="600" color="gray.800">No assigned students</Text>
                      <Text fontSize="xs" color="gray.500" mt="1">This lecturer hasn't been assigned any students yet.</Text>
                      <Button size="sm" colorScheme="blue" mt="4" onClick={() => setShowAssigned(false)}>Back to Unassigned</Button>
                    </Box>
                  </Flex>
                ) : (
                  <>
                    ...
                    <For each={filteredAssignedStudents}>
                      {(student) => (
                        <Box key={student.id} border="1px solid" borderColor="gray.100" bg="white" rounded="lg" p="4" _hover={{ bg: "gray.50" }} transition="all 0.2s">
                          <Flex align="flex-start" justify="space-between">
                            <Box flex="1">
                              <Text fontWeight="700" fontSize="sm" color="gray.800" mb="0.5">{student.name || student.fullName}</Text>
                              <Text fontSize="xs" color="gray.500" fontWeight="500">{student.email}</Text>
                              <Flex align="center" gap="2" mt="2">
                                <Box px="2" py="0.5" bg="gray.100" rounded="full">
                                  <Text fontSize="10px" fontWeight="700" color="gray.600">{student.matricNumber}</Text>
                            ...
                            <Button size="xs" colorScheme="red" variant="ghost" onClick={() => setStudentToRemove(student)} loading={isRemoving && studentToRemove?.id === student.id}>
                              Remove
                            </Button>
                          </Flex>
                        </Box>
            ...
            <Dialog.Content rounded="xl" maxW="400px">
              ...
              <Dialog.Footer borderTopWidth="1px" borderColor="gray.100" py="4">
                <Flex gap="3" justify="flex-end">
                  ...
                  <Button colorScheme="blue" onClick={handleRemoveStudent} loading={isRemoving} bg="blue.600" _hover={{ bg: "blue.700" }}>Remove</Button>
                </Flex>
              </Dialog.Footer>
